...
import _ from 'lodash'; // Using lodash for deep comparison

/**
 * Compares two V2 EmailTemplate structures and generates a diff result.
 */
export class DifferV2 {
  ...
  private diffSections(oldSections: EmailSection[], newSections: EmailSection[]): SectionDiff[] {
    ...
    // TODO: Potentially re-order diffs based on the new index order?
    // For now, the order reflects processing old sections first, then new ones.
             ...
             if (
                ...
                // Add a depth check here if full recursion was needed
                 ...
                 nestedKeys.forEach(nestedKey => {
                     if (!_.isEqual(oldValue[nestedKey], newValue[nestedKey])) {
